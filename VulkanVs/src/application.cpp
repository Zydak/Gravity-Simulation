...
static const char* Skyboxes[] = { "Milky Way", "Nebula", "Stars", "Red Galaxy"};// I tried to make this a map but imgui only works with c-string array
...
void Application::Run()
{
    ...
    while(!m_Window.ShouldClose())
    {
        ...
        if (auto commandBuffer = m_Renderer->BeginFrame())
        {
            ...
            {
                Update(frameInfo, 60.0); // making delta value larger will speed up simulation while loosing its accuracy but I guess making it 0.016 and waiting 10 thousand days for some orbit to complete is not good idea so it is necessery to do that
            ...
            m_Camera.SetViewTarget({0.0, 0.0, 0.0}); // it is (0,0,0) beacuse of floating-origin method which basically instad of moving camera to objects
                          // at large offsets moves everything to camera at (0,0,0). It is necessery on large scales like here to properly display objects
